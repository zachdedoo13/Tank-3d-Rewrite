...
func check_direction(position:Vector3, direction:float, draw=true, collision_masks=[2], debug_look_vector=null) -> Array:
	...
	var target = Vector3(look_vector.y, position.y, look_vector.x)
	...
	for i in range(len(collision_masks)):
		raycast.set_collision_mask_value(collision_masks[i], true)
...
func new_check_direction_bounce(position:Vector3, direction:float, draw=true, collision_masks=2) -> Array:
	# is collideing | collider | collision point | line1 | line2 |
	var ret = [false, null, false, Node3D.new(), Node3D.new()]
	
	var first_ray = check_direction(position, direction, draw, collision_masks)
	...
	if first_ray[0]:
		...
		var second_ray = check_direction(first_ray[2], 0, draw, collision_masks, bounce_look_vector)
