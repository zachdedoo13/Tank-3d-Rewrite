...
func look_and_check() -> void:
	var store = RayTool.new_check_direction_bounce(global_position, rotation.y, false, [5, 7])
	if store[0]:
	...
	#
	#RayTool.clear_check_direction(line, 100)
	#RayTool.clear_check_direction(line2, 100)
